...
st.set_page_config(page_title="Distress Scoring Test", layout="wide")
st.title("Distress Scoring Test")

# --- Address Parsing ---
def parse_address(row):
    if 'address1' in row and 'address2' in row:
        return row['address1'], row['address2']
    elif 'address' in row:
        # Try to split full address into address1/address2
...
# --- Async ATTOM API Calls ---
async def fetch_attom(client: httpx.AsyncClient, endpoint: str, params: dict) -> dict:
...
async def analyze_row(row: dict, client: httpx.AsyncClient) -> dict:
    address1, address2 = parse_address(row)
    ...
    # 12. Hazard area (not implemented, placeholder)
    # TODO: Add hazard area check if you have endpoint/data
    # ---
...
# --- Streamlit UI ---
uploaded = st.file_uploader("Upload Excel with address column(s)", type=["xlsx", "xls", "csv"])
if uploaded:
    ...
    if st.button("Run Distress Scoring"):
        ...
        def run_analysis(df):
            async def main():
                async with httpx.AsyncClient() as client:
                    tasks = [analyze_row(row, client) for row in df.to_dict(orient='records')]
        ...
        st.download_button("Export CSV", out_df.to_csv(index=False), file_name="distress_scored_leads.csv") 